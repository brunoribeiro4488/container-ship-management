...
public class BSTIMO extends BST{
    /** Nested static class for a binary search tree node. */

    public Ship find(String imo){
        if (isEmpty()){
            return null;
        }else {
            BST.Node<Ship> result = find(root(), imo);
            if (result == null){
                return null;
    ...
    protected BST.Node<Ship> find(BST.Node<Ship> node, String element){
        ...
        if (node.getElement().compareToIMO(element)==0){
            return node;
        }else if (node.getElement().compareToIMO(element)==1){
    ...
    private BST.Node<Ship> insert(Ship element, BST.Node<Ship> node){
        ...
        if (node.getElement().compareToIMO(element.getImo()) == 1) {
            node.setLeft(insert(element, node.getLeft()));
        } else if (node.getElement().compareToIMO( element.getImo())==-1 ) {
