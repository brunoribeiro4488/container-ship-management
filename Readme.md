...
Besides, students can only add dependencies to the specified section of this file.
...
$ mvn test jacoco:report org.pitest:pitest-maven:mutationCoverage \
  -DhistoryInputFile=target/fasterPitMutationTesting-history.txt \
  -DhistoryOutputFile=target/fasterPitMutationTesting-history.txt \
  -Dsonar.pitest.mode=reuseReport -Dthreads=4 -DtimestampedReports=false
```
